...
def genere_bateau(plateau, type_bateau, numero_bateau):
    ...
    """
    assert (type_bateau == 'Porte-avions') or (type_bateau ==
                                               'Croiseur') or (type_bateau == 'Destroyer') or (type_bateau == 'Torpilleur')
    ...
    while (placement_possible(plateau, position, nb_cases, orientation) == False):
        orientation, position = genere_position_aleatoire(
            len(plateau), nb_cases)

    if (orientation == 'H'):
        for j in range(nb_cases):
            plateau[position[0]][position[1] +
                                 j] = {'contenu': type_bateau, 'numero': numero_bateau, 'etat': 'Neuf'}
    else:
        for i in range(nb_cases):
            plateau[position[0]+i][position[1]
                                   ] = {'contenu': type_bateau, 'numero': numero_bateau, 'etat': 'Neuf'}
        ...
        for colonne in ligne:
            if (colonne['etat'] == 'Neuf'):
                ...
                if (colonne['contenu'] == 'Eau'):
                    ...
                    a_afficher = a_afficher + \
                        '{}'.format(colonne['contenu'][0].lower())
        a_afficher = a_afficher + '|'
        print(a_afficher)
    print('-'*(len(plateau) + 2))
    ...
    if (plateau[position[0]][position[1]]['etat'] == 'Touche'):
        ...
        if (plateau[position[0]][position[1]]['contenu'] == 'Eau'):
            ...
            if (verif_coule(plateau, plateau[position[0]][position[1]]['contenu'], plateau[position[0]][position[1]]['numero'])):
                print(plateau[position[0]][position[1]]['contenu'],
                      plateau[position[0]][position[1]]['numero'], 'coulé')
            else:
                print(plateau[position[0]][position[1]]['contenu'],
                      plateau[position[0]][position[1]]['numero'], 'touché')
...
def sauvegarder_partie(plateau, taille_grille, nombre_tir):
    taille_grille = 10
    nombre_tir = 0
